...
plural.__doc__ = (
    "Helper function that returns singular or plural word "
...
def get_stats(txt):
    ...
    for char in txt:
        if char.isupper():
            ...
            stats["punctuation"] += 1
    print(
        f"The text contains {stats['character']} "
        f"{plural('character', stats['character'])}:"
    )
...
get_stats.__doc__ = (
    "Helper function to process text and count chars, "
    "upper letters, lower, spaces, digits and punctuation marks"
)
...
main.__doc__ = (
    "main function that reads text from args or stdin "
    "and process it with helper function get_stats()"
)
